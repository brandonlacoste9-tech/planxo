...
export async function POST(request: Request) {
  try {
    ...
    const { eventTypeId, email, name, startTime, endTime, paymentMethod = "INTERAC" } = body;
    ...
    const basePrice = eventType.price || 0; // Baseline cost from your DB models (in cents)
    const tpsRate = 0.05;       // Federal GST
    const tvqRate = 0.09975;    // Quebec QST
    ...
    const newBooking = await prisma.$transaction(async (tx) => {
      ...
      const booking = await tx.booking.create({
        data: {
          ...
          userId: eventType.userId, // Required by schema
          guestName: name,
          ...
          paymentStatus: paymentMethod === "INTERAC" ? "UNPAID" : "PAID",
